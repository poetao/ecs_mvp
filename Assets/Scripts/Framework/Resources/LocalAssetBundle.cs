...
namespace MVP.Framework.Resources
{
    public class LocalAssetBundle : Resource
    {
        ...
        protected override async Task<AssetBundle> LoadAssetBundleSingleAsync(string path, LoadingOption option, float min = 0, float max = 1.0f)
        {
            ...
            var result = await Loading.instance.Wrap(
                async () => { return await HookProgress(createRequest, option.progress, min, max); }, option);
