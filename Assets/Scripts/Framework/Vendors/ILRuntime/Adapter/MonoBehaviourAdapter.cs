...
public class MonoBehaviourAdapter : CrossBindingAdaptor
{
    ...
    public class Adaptor : MonoBehaviour, CrossBindingAdaptorType
    {
        ...
        IMethod mTriggerEnter2DMethod;
        private void OnTriggerEnter2D(Collider2D other)
        {
            if (!mOnTriggerEnter2DMethodGot)
            {
                mTriggerEnter2DMethod = instance.Type.GetMethod("OnTriggerEnter2D", 1);
                mOnTriggerEnter2DMethodGot = true;
            }

            if (mTriggerEnter2DMethod != null)
            {
                appdomain.Invoke(mTriggerEnter2DMethod, instance, other);
            }
        }

        bool mOnTriggerExit2DMethodGot;
        IMethod mTriggerExit2DMethod;
        private void OnTriggerExit2D(Collider2D other)
        {
            if (!mOnTriggerExit2DMethodGot)
            {
                mTriggerExit2DMethod = instance.Type.GetMethod("OnTriggerExit2D", 1);
                mOnTriggerExit2DMethodGot = true;
            }

            if (mTriggerExit2DMethod != null)
            {
                appdomain.Invoke(mTriggerExit2DMethod, instance, other);
            }
        }
